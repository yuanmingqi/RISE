...
def simulate():
    ...
    for episode in range(NUM_EPISODES):
        ...
        for t in range(MAX_T):
            ...
            state_visitation_counter.append(state[0] * MAZE_SIZE[0] + state[1])
            ...
            if done:
                num_visited_states = np.unique(state_visitation_counter).shape[0]
                total_exploration_steps += t
                ...
                if num_visited_states == np.prod(MAZE_SIZE):
                    print('Total exploration steps ', total_exploration_steps)
                    exit(0)
